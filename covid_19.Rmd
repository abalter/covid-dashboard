...
covid_infection_folding %>%
  ...
  geom_smooth(aes(x=Date, y=US_Infections, color="US Infections")) +
  geom_smooth(aes(x=Date, y=US_Deaths, color="US Deaths")) +
  labs(y="Doubling Time in Days")
```

## World Infections and Deaths
```{r}
covid_infection_folding %>%
  ggplot() + 
  geom_smooth(aes(x=Date, y=World_Infections, color="World Infections")) +
  geom_smooth(aes(x=Date, y=World_Deaths, color="World Deaths")) +
  labs(y="Doubling Time in Days")
```
...
covid_infection_folding %>%
  ...
  labs(
    ...
    subtitle="Based on purevious 7 days"
  )
```
### World
```{r}
covid_infection_folding %>%
  ...
  # scale_x_date(date_breaks = "month", date_minor_breaks="day", date_labels="%b %e") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  labs(
    y="Doubling Time (Days)",
    title="World Covid-19 Doubling Time (2020)",
    subtitle="Based on previous 7 days. 95% confidence intervals."
  )
```
